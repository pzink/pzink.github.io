...
let ps=$(identify -format '%w' surfcam-image.jpg)/20
# 2023:10:04 09:29:24
datetime=$(identify -format  %[exif:datetime] surfcam-image.jpg)
if [ -z "$datetime" ]
...
magick surfcam-image.jpg -fill black  -undercolor white -pointsize "$ps" -gravity southeast \
 -annotate 0 "$datetime" surfcam-image.jpg

let ps=$(identify -format '%w' ocean-image.jpg)/20
# 2023:10:04 09:29:24
datetime=$(identify -format  %[exif:datetime] ocean-image.jpg)
if [ -z "$datetime" ]
then    # date:create: 2023-11-11T14:40:21+00:00
        datetime=$(identify -format  %[date:create] ocean-image.jpg)
fi
datetime=$(echo "$datetime" | gawk -F'[- :T+]' '{print $3"-"$2"-"$1 " " $4":"$5}' )
magick ocean-image.jpg -fill black  -undercolor white -pointsize "$ps" -gravity southeast \
 -annotate 0 "$datetime" ocean-image.jpg
